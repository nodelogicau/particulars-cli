...
func (a *app) claimCmd() *cobra.Command {
	...
	cmd.AddCommand(a.claimAssertCmd(), a.claimRetractCmd())
	return cmd
}
...
func (a *app) claimAssertCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: a.run(func(cmd *cobra.Command, args []string) error {
			...
			if src.Author == "" {
				return usageErr("source.author is required: pass --author, set %s, or configure defaults.source.author in dkf.yaml", EnvAuthor)
			}
...
func (a *app) claimRetractCmd() *cobra.Command {
	var reason, supersededBy string
	var prov provenanceFlags
	cmd := &cobra.Command{
		Use:   "retract <id> --reason <text> [--superseded-by <id>] [flags]",
		Short: "Mark a claim or synthesis retracted (append-only; never deletes)",
		...
		RunE: a.run(func(cmd *cobra.Command, args []string) error {
			...
			if !dkf.IsAssertionID(id) {
				return usageErr("%q is not a claim or synthesis id", id)
			}
			ws, err := a.openWorkspace()
			if err != nil {
				return err
	...
	cmd.Flags().StringVar(&reason, "reason", "", "why the claim is retracted (required)")
	cmd.Flags().StringVar(&supersededBy, "superseded-by", "", "id of the claim or synthesis that replaces it")
	cmd.Flags().StringVar(&prov.author, "author", "", "who is retracting (default: $DKF_AUTHOR, then dkf.yaml)")
	cmd.Flags().StringVar(&prov.harness, "harness", "", "harness performing the retraction")
