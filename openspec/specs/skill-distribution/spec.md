...
The agent-facing skill embedded in the binary: `skill show`, `skill install` (project, user, or explicit directory), version stamping and the ownership marker, the never-overwrite-a-foreign-file rule, and the `--check` drift mode for CI.
