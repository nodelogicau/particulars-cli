...
- **THEN** the file carries a `document` mapping with those three keys in the order `ref`, `hash`, `quote`

#### Scenario: ref is required in the mapping form
- **WHEN** a document mapping carries `hash` or `quote` but no `ref`
