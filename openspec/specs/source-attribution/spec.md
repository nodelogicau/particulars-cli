...
Author values as particular references: how a reader resolves `source.author` and `source.document.author` (id, URI through merges, bare name), how a writer resolves them for writing and what it refuses, the asserted-by and reported-from relations over merge equivalence classes, and the aggregate author findings.
