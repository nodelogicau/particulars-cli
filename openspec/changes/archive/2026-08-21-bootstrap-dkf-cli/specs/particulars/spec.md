...
`particulars particular resolve <query>` SHALL return all particulars whose `id` or `uri` equals the query exactly, or whose `label` or any alias equals the query case-insensitively. The result SHALL be `{"matches": [...]}` in JSON mode. If no particular matches, the command SHALL exit with code 3.
