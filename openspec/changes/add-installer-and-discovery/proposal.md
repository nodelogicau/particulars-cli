...
- **Homebrew tap**: GoReleaser publishes a formula to `nodelogicau/homebrew-tap` on each release, so `brew install nodelogicau/tap/particulars` works. Requires a tap repository and a `HOMEBREW_TAP_GITHUB_TOKEN` secret (user-side setup, listed in tasks).
...
- New files: `install.sh`, `.github/workflows/installer.yml` (or a job in `ci.yml`), `internal/cli/cmd_workspace.go`; `.goreleaser.yaml` gains `brews`; `release.yml` passes the tap token.
